...
const ChatSearchModal = ({open, onClose, mode, onAddMembers}: ChatSearchModalProps) => {
	...
	return (
		<Dialog open={open} onOpenChange={onClose}>
			<DialogContent className="max-w-md">
				<DialogHeader>
					<DialogTitle>{mode === "private" ? "Start Private Chat" : "Create Group Chat"}</DialogTitle>
				</DialogHeader>
								...
								mode === "private" ? (
									...
									<div
										...
										onClick={async () => {
											...
											setQuery("");
											setResults([]);
											await handleCreatePrivateChat(r.id!); // pass the user id
											onClose();
										}}>
										{r.username}
									</div>
									...
									>
										<input
											...
											onChange={(e) => {
												if (e.target.checked) {
													setSelectedGroupMembers((prev) => r.id ? [...prev, r.id] : [...prev]);
												} else {
