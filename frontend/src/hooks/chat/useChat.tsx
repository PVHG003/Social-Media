...
interface ChatContextType {
  ...
  addChat: (chat: ChatDetailResponse | undefined) => void;
...
export const ChatProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const selectChat = (chatId: string) => {
    ...
    fetchChatMessages(chatId, { page: 0, size: messageSize })
      ...
      .catch((err: Error) =>
        console.error("[selectChat] Reset messages from chat:", err)
      );
  };
    ...
    try {
      ...
      setChats((prev) => {
        ...
        const sorted = merged.sort((a, b) => {
          const timeA = a.lastMessageSentAt
            ? new Date(a.lastMessageSentAt).getTime()
            : 0;
          const timeB = b.lastMessageSentAt
  ...
  const fetchChatMessages = async (
    ...
    try {
      ...
      console.log(
        `[fetchChatMessages] Received ${newMessages.length} messages`,
        newMessages
      );
  ...
  const updateChatList = (message: ChatMessageResponse, chatId: string) => {
    setChats((prev) => {
      ...
      if (!existingChat) {
        // New chat → set defaults for missing fields
        const newChat: ChatListResponse = {
          chatId,
          chatDisplayName: message.senderUsername, // fallback until we fetch details
          chatDisplayImage: message.senderProfileImage,
          chatType: undefined,
  ...
  const addChat = (chat: ChatDetailResponse | undefined) => {
    ...
    setChats((prev) => {
      ...
      const newChat: ChatListResponse = {
        ...
        chatDisplayImage: chat.chatDisplayImage,
        chatType: chat.chatType as ChatListResponseChatTypeEnum,
        lastMessage: undefined,
        lastMessageSenderUsername: undefined,
        lastMessageSentAt: chat.createdAt,
        unreadMessagesCount: 0,
        muted: false,
        memberIds: chat.members?.map((m) => m.id!) ?? [],
